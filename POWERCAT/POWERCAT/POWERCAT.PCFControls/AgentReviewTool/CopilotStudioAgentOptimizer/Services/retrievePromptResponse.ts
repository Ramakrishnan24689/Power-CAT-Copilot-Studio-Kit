...
interface RetrievePromptResponseInput {
	...
	runtime?: 'codeinterpreter' | null; // null = AI Prompt mode, 'codeinterpreter' = Code Interpreter mode
}
...
interface PredictionOutput {
	text?: string;
	structuredOutput?: StructuredOutput | Record<string, unknown>;
	files?: FileOutput[];
}
...
export async function retrievePromptResponse({
	...
	modelId,
	requestInputs,
	runtime = null, // Default to AI Prompt mode (null)
	...
	const runtimeMode = runtime === null ? '🧠 AI Prompt' : `🔧 ${runtime}`;
	console.log(`🚀 [STAGE B AI] 📥 INPUT: ${runtimeMode} mode - Model ${modelId}`);
	console.log(`🚀 [STAGE B AI] 📋 Request Inputs:`, requestInputs);

	const body = {
		version: "2.0",
		source: "{ \"consumptionSource\": \"Api\", \"partnerSource\": \"PVA\", \"consumptionSourceVersion\": \"GptApiClient\"}", // Required source field
		requestv2: {
			...
			$customConfig: {
				...
				settings: {
					"@odata.type": "#Microsoft.Dynamics.CRM.expando",
					runtime: runtime, // null = AI Prompt, "codeinterpreter" = Code Interpreter
				},
			},
		},
	};
	...
	try {
		...
		console.log(`🚀 [STAGE B AI] 📤 OUTPUT: Success - Response received`);
		console.log(`🚀 [STAGE B AI] 📋 Response Data:`, data.responsev2);
		if (data.error) {
			console.error(`🚀 [STAGE B AI] ❌ API Error:`, data.error);
		}
		return data;
	} catch (error) {
