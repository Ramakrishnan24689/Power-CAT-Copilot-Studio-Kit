...
function hideAndShowConversationKPISettings(executionContext) {
    ...
    const configurationTypeValues = formContext
        .getAttribute("cat_configurationtypescodes")
        .getValue() || [];
    ...
    const fileConfigDetailsSection = tabGeneral.sections.get("tab_general_section_file_config_details"); 
    const conversationTranscriptsSection = tabGeneral.sections.get("tab_general_section_conversationtranscriptsenrichment");
    ...
    if (configurationTypeValues.includes(2)) {
        ...
        setFieldRequirements(formContext, ["cat_copilotid", "cat_dataverseurl"], "none");
    }
...
function setFieldVisibilityForEachSections(executionContext) {
    const formContext = executionContext.getFormContext();
    const configurationTypeValues = formContext.getAttribute("cat_configurationtypescodes").getValue() || [];

    // User Authentication Fields Rules
    const userAuth = formContext.getAttribute("cat_userauthenticationcode").getValue();

    // Entra ID v2 and Test Automation
    if (userAuth === 2 && configurationTypeValues.includes(1)) { 
        setFieldVisibility(formContext, ["cat_clientid", "cat_tenantid", "cat_scope"], true, "required");

    // No Authentication and Test Automation
    } else if (userAuth === 1 && configurationTypeValues.includes(1)) { 
        clearAndHideFields(formContext, ["cat_clientid", "cat_tenantid", "cat_scope"]);
    }

    // Enrich With Azure Application Insights Secret Location Rule
    const secretLocation = formContext.getAttribute("cat_azureappinsightssecretlocationcode").getValue();
    if (secretLocation === 1 && configurationTypeValues.includes(1)) {
        setFieldVisibility(formContext, ["cat_azureappinsightssecret"], true, "required");
        clearAndHideFields(formContext, ["cat_azureappinsightsenvironmentvariable"]);
    } else if (secretLocation === 2 && configurationTypeValues.includes(1)) {
        setFieldVisibility(formContext, ["cat_azureappinsightsenvironmentvariable"], true, "required");
        clearAndHideFields(formContext, ["cat_azureappinsightssecret"]);
    } else {
        clearAndHideFields(formContext, ["cat_azureappinsightssecret", "cat_azureappinsightsenvironmentvariable"]);
    }

    // Enrich With Azure Application Insights Fields Rules
    const enrichWithAI = formContext.getAttribute("cat_isazureapplicationinsightsenabled").getValue();
    if (enrichWithAI === true && configurationTypeValues.includes(1)) {
        setFieldVisibility(formContext, ["cat_azureappinsightsapplicationid", "cat_azureappinsightstenantid", "cat_azureappinsightsclientid", "cat_azureappinsightssecretlocationcode"], true, "required");
    ...
    const dlSecurity = formContext.getAttribute("cat_isdirectlinechannelsecurityenabled").getValue();
    if (dlSecurity === true && configurationTypeValues.includes(1)) {
        setFieldVisibility(formContext, ["cat_directlinechannelsecretlocationcode"], true, "required");
        clearAndHideFields(formContext, ["cat_tokenendpoint"]);
    } else if (dlSecurity === false && configurationTypeValues.includes(1)) {
        clearAndHideFields(formContext, ["cat_directlinechannelsecretlocationcode"]);
        setFieldVisibility(formContext, ["cat_tokenendpoint"], true, "required");
    }

    // Direct Line Channel Security Secret Location Fields Rules
    const dlSecretLocation = formContext.getAttribute("cat_directlinechannelsecretlocationcode").getValue();
    if (dlSecretLocation === 1 && configurationTypeValues.includes(1)) {
        setFieldVisibility(formContext, ["cat_directlinechannelsecuritysecret"], true, "required");
        clearAndHideFields(formContext, ["cat_directlinechannelsecurityenvironmentvariable"]);
    } else if (dlSecretLocation === 2 && configurationTypeValues.includes(1)) {
        setFieldVisibility(formContext, ["cat_directlinechannelsecurityenvironmentvariable"], true, "required");
        clearAndHideFields(formContext, ["cat_directlinechannelsecuritysecret"]);
    } else {
        clearAndHideFields(formContext, ["cat_directlinechannelsecuritysecret", "cat_directlinechannelsecurityenvironmentvariable"]);
    }

    // Analyze Generated Answers Fields Rules
    const analyzeAnswers = formContext.getAttribute("cat_isgeneratedanswersanalysisenabled").getValue();
    if (analyzeAnswers === true && configurationTypeValues.includes(1)) {
        setFieldVisibility(formContext, ["cat_generativeaiprovidercode"], true, "required");
    } else {
    ...
    if (isEnrichedWithTranscripts === true && configurationTypeValues.includes(1)) {
